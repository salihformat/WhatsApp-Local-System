<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('local_agent.print_rules_title') }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            @if($printers->isEmpty())
                <div class="bg-yellow-100 border border-yellow-400 text-yellow-800 px-4 py-3 rounded relative">
                    {!! __('local_agent.print_rules_need_printer', ['link' => '<a href="' . route('printers.index') . '" class="underline font-bold">' . __('local_agent.add_a_printer') . '</a>']) !!}
                </div>
            @endif
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6 mb-6">
                ...
                <form action="{{ route('print-rules.store') }}" method="POST" class="grid grid-cols-1 md:grid-cols-6 gap-6 items-end">
                    ...
                    <div class="md:col-span-2">
                        <label class="block text-sm font-bold text-gray-700 mb-2">{{ __('local_agent.rule_name') }}</label>
                        <input type="text" name="name" required class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" placeholder="{{ __('local_agent.rule_name_placeholder') }}">
                    </div>

                    <div class="md:col-span-2">
                        <label class="block text-sm font-bold text-gray-700 mb-2">{{ __('local_agent.match_type') }}</label>
                        <select name="match_type" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            ...
                            <option value="keyword">{{ __('local_agent.match_keyword') }}</option>
                            <option value="file_type">{{ __('local_agent.match_file_type') }}</option>
                        </select>
                    </div>

                    <div class="md:col-span-2">
                        <label class="block text-sm font-bold text-gray-700 mb-2">{{ __('local_agent.match_value') }} <span class="text-xs text-gray-400 font-normal">({{ __('local_agent.match_value_hint') }})</span></label>
                        <input type="text" name="match_value" required class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" placeholder="{{ __('local_agent.match_value_placeholder') }}">
                    </div>
                        ...
                        @forelse($rules as $rule)
                            <tr class="hover:bg-gray-50 transition-colors">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-center text-gray-600">
                                    @php
                                        $labels = ['phone_number' => __('local_agent.condition_phone_eq'), 'phone_prefix' => __('local_agent.condition_prefix_eq'), 'keyword' => __('local_agent.condition_contains_word'), 'file_type' => __('local_agent.condition_ext_eq')];
                                    @endphp
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    <div class="flex items-center justify-center gap-2" x-data="{ editModalOpen: false }">
                                        ...
                                        <!-- Delete Button -->
                                        <form action="{{ route('print-rules.destroy', $rule) }}" method="POST" class="inline" onsubmit="return confirm('{{ __('local_agent.confirm_delete_rule') }}');">
                                            @csrf @method('DELETE')
                                                    ...
                                                    <form action="{{ route('print-rules.update', $rule) }}" method="POST">
                                                        ...
                                                        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4 text-right whitespace-normal text-base">
                                                            <h3 class="text-lg leading-6 font-bold text-indigo-700 mb-6 border-b pb-3" id="modal-title">
                                                                {{ __('local_agent.edit_rule') }}: {{ $rule->name }}
                                                            </h3>
                                                            <div class="space-y-4 text-right">
                                                                ...
                                                                <div>
                                                                    <label class="block text-sm font-bold text-gray-700 mb-2">{{ __('local_agent.match_type') }}</label>
                                                                    <select name="match_type" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                                                        <option value="phone_number" {{ $rule->match_type == 'phone_number' ? 'selected' : '' }}>{{ __('local_agent.match_phone_number') }}</option>
                                                                        <option value="phone_prefix" {{ $rule->match_type == 'phone_prefix' ? 'selected' : '' }}>{{ __('local_agent.match_phone_prefix') }}</option>
                                                                        <option value="keyword" {{ $rule->match_type == 'keyword' ? 'selected' : '' }}>{{ __('local_agent.match_keyword') }}</option>
                                                                        <option value="file_type" {{ $rule->match_type == 'file_type' ? 'selected' : '' }}>{{ __('local_agent.match_file_type') }}</option>
                                                                    </select>
                                                                </div>

                                                                <div>
                                                                    <label class="block text-sm font-bold text-gray-700 mb-2">{{ __('local_agent.match_value') }} <span class="text-xs text-gray-400 font-normal">({{ __('local_agent.match_value_hint_short') }})</span></label>
            ...
            <div class="flex gap-4">
                <a href="{{ route('printers.index') }}" class="text-indigo-600 hover:underline">{{ app()->getLocale() === 'ar' ? '→' : '←' }} {{ __('local_agent.manage_printers') }}</a>
