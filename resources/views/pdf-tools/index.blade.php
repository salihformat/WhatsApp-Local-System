    ...
    <div class="py-12">
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6" x-data="pdfMerger()">
                <h3 class="text-lg font-bold mb-2 text-indigo-700">{{ __('local_agent.pdf_merge_title') }}</h3>
                <p class="text-sm text-gray-500 mb-4">{{ __('local_agent.pdf_merge_help') }}</p>
                <form action="{{ route('pdf-tools.merge') }}" method="POST" enctype="multipart/form-data" @submit="submitForm">
                    ...
                    <div class="mb-4">
                        ...
                        <button type="button" @click="document.getElementById('pdfFileInput').click()" class="bg-gray-100 text-gray-700 px-4 py-2 rounded-md border border-gray-300 hover:bg-gray-200 text-sm font-medium transition-colors inline-flex items-center gap-2">
                            ...
                            {{ __('local_agent.pdf_add_files') }}
                        </button>
                    </div>

                    <div class="mb-4 space-y-2" x-show="files.length > 0" x-cloak>
                        <label class="block text-sm font-medium text-gray-700">{{ __('local_agent.pdf_selected_files') }}</label>
                    ...
                    <div class="flex justify-end">
                        <button type="submit" :disabled="files.length < 2" :class="{'opacity-50 cursor-not-allowed': files.length < 2}" class="bg-indigo-600 text-white px-5 py-2 rounded-md hover:bg-indigo-700 transition-colors whitespace-nowrap text-sm font-medium">{{ __('local_agent.pdf_merge_button') }}</button>
                    </div>
                </form>
            </div>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <h3 class="text-lg font-bold mb-2 text-indigo-700">{{ __('local_agent.pdf_split_title') }}</h3>
                <p class="text-sm text-gray-500 mb-4">{{ __('local_agent.pdf_split_help') }}</p>
                <form action="{{ route('pdf-tools.split') }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 sm:grid-cols-4 gap-4 items-end">
                    ...
                    <div>
                        <label class="block text-xs text-gray-600 mb-1">{{ __('local_agent.pdf_from_page') }}</label>
                        <input type="number" name="from_page" min="1" value="1" required class="w-full rounded-md border-gray-300 shadow-sm text-sm">
                    </div>
                    <div>
                        <label class="block text-xs text-gray-600 mb-1">{{ __('local_agent.pdf_to_page') }}</label>
                        <input type="number" name="to_page" min="1" value="1" required class="w-full rounded-md border-gray-300 shadow-sm text-sm">
                    </div>
                    <div class="sm:col-span-4 flex justify-end">
                        <button type="submit" class="bg-indigo-600 text-white px-5 py-2 rounded-md hover:bg-indigo-700 text-sm font-medium">{{ __('local_agent.pdf_split_button') }}</button>
                    </div>
                </form>
            </div>
            ...
            <!-- ضغط صورة -->
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <h3 class="text-lg font-bold mb-2 text-indigo-700">{{ __('local_agent.pdf_compress_title') }}</h3>
                <p class="text-sm text-gray-500 mb-4">{{ __('local_agent.pdf_compress_help') }}</p>
                <form action="{{ route('pdf-tools.compress-image') }}" method="POST" enctype="multipart/form-data" class="flex flex-col sm:flex-row gap-4 items-start sm:items-end">
                    ...
                    <div>
                        <label class="block text-xs text-gray-600 mb-1">{{ __('local_agent.pdf_quality') }}</label>
                        <input type="number" name="quality" min="10" max="95" value="60" class="w-24 rounded-md border-gray-300 shadow-sm text-sm">
                    </div>
                    <button type="submit" class="bg-indigo-600 text-white px-5 py-2 rounded-md hover:bg-indigo-700 whitespace-nowrap text-sm font-medium">{{ __('local_agent.pdf_compress_button') }}</button>
                </form>
            </div>
        </div>
    </div>
            ...
            Alpine.data('pdfMerger', () => ({
                ...
                submitForm(event) {
                    if (this.files.length < 2) {
                        ...
                        alert({{ Js::from(__('local_agent.pdf_merge_min_files')) }});
                        return;
                    }
