...
return [
    ...
    // رقم (أو أرقام، مفصولة بفواصل) واتساب المسؤول — يصله تنبيه فوري عند تغيّر حالة أي طابعة
    // (توقف/نفاد ورق أو حبر/انحشار...)، طلبات موافقة الطباعة/الإرسال، تذكيراتها، وتنبيهات صحة النظام.
    // مثال لأكثر من رقم: "966501111111,966502222222" — أي رقم منهم يستلم كل التنبيهات، ويمكنه الرد
    // بأوامر الموافقة (وافق/رفض/...) لأي طلب. اتركه فارغاً لتعطيل كل هذه التنبيهات (تبقى مرئية في
    // لوحة التحكم رغم ذلك). راجع App\Services\AdminNotifier — نقطة الإرسال الموحّدة لكل ما سبق.
    ...
    // [New] عندما تصل رسالة نصية منفصلة عن الملف (مثال: "طباعة") تُطابق قاعدة كلمة مفتاحية، يُطبَّق
    // إجراؤها على آخر ملف وارد من نفس رقم المرسل خلال هذه المدة (بالدقائق) فقط — نافذة زمنية تمنع نص
    // متأخر لاحقاً بأشهر من إعادة طباعة ملف قديم منسي.
    'last_file_command_window_minutes' => (int) env('PRINTING_LAST_FILE_COMMAND_WINDOW_MINUTES', 30),
    ...
    // بعد كم دقيقة من بقاء طلب موافقة (طباعة أو إرسال) بلا رد، يُرسَل تذكير تلقائي للمسؤول؟
    // يفحصه أمر printing:send-approval-reminders المجدول كل 10 دقائق (راجع routes/console.php).
    'approval_reminder_after_minutes' => (int) env('PRINTING_APPROVAL_REMINDER_AFTER_MINUTES', 20),

    // بعد أول تذكير، كل كم دقيقة يُعاد إرساله ما دام الطلب لا يزال معلّقاً؟ (تفادي إزعاج المسؤول
    // بتذكير كل 10 دقائق إلى ما لا نهاية). اضبطه على 0 لتعطيل التذكير التلقائي كلياً.
    'approval_reminder_repeat_minutes' => (int) env('PRINTING_APPROVAL_REMINDER_REPEAT_MINUTES', 30),
];
